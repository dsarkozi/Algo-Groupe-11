...
/**
 * Classe qui implemente les reponses a la question 9 au sujet
 * du taux de compression de nos programmes.
 * @author clementine
 *
 */
public class Q9 {
	...
	public static void main(String[] args) throws IOException {
		...
		/**
		 * Test influence nombre de chars différents dans le texte:
		 * on crée 9 fichiers de meme taille (approximativement 2000) ou on fait augmenter de façon 
		 * constante le nombre de charactères différents
		 */
		...
		for(int i = 0; i<8; i++){
			...
			br = new BufferedReader(new FileReader("file"+i)); //TODO put cfile
			while (br.read() != -1){
				tc ++;
			}
			br.close();
		...
		/**
		 * Test influence taille du fichier:
		 * on crée et compresse 10 fichiers de taille constamment croissantes (+10000)
		 * et on calcule le taux de comression pour chaque fichier ainsi que le temps
		 * d'execution de chaque compression.
		 * On s'assure de garder le nombre de caractère différents et la variance des nombres 
		 * d'occurences des caracteres constants  
		 */
		final int X = 10;
		o = 10000;
		tc = 0;
		...
		for(int i = 0; i<X; i++){
			...
			tc = 0;
			br = new BufferedReader(new FileReader("file2"+i));//TODO put cfile2
			while (br.read() != -1){
				tc ++;
			}
			br.close();
			...
			System.out.println((o*X)+"chars\t\t\t"
					+taux2.get(i)+"\t\t\t"+temps.get(i)+"\t\t\t"+dctemps.get(i));
